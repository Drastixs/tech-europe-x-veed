...
class TutorialActionBase(ContractModel):
...
class MoveParameters(ContractModel):
    duration_ms: Annotated[int, Field(ge=0, le=10_000)]


class PointerParameters(ContractModel):
...
class KeypressParameters(ContractModel):
    key: str = Field(min_length=1)
    modifiers: list[Literal["alt", "control", "meta", "shift"]]
    repeat: Annotated[int, Field(ge=1, le=100)]


class TypeParameters(ContractModel):
    text: str = Field(min_length=1)
    clear_existing: bool
    submit: bool


class ScrollParameters(ContractModel):
    delta_x: int
    delta_y: int
    duration_ms: Annotated[int, Field(ge=0, le=10_000)]
...
class WaitParameters(ContractModel):
...
class SelectionParameters(ContractModel):
